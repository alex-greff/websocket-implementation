...
var WebSocketFrame = /** @class */ (function () {
    ...
    WebSocketFrame.fromBuffer = function (buffer) {
        ...
        var bitBuffer = new BitView(buffer);
        bitBuffer.bigEndian = true;
        var bufferRemainingBits = buffer.length * 8;
        // Get fin value
        var fin = intToBool(bitBuffer.getBits(BITS_OFFSET_FIN, BITS_LEN_FIN, false));
        bufferRemainingBits -= BITS_LEN_FIN;
        // Ensure RSV values are 0
        var rsv = bitBuffer.getBits(BITS_OFFSET_RSV, BITS_LEN_RSV, false);
        bufferRemainingBits -= BITS_LEN_RSV;
        if (rsv !== 0)
            throw new WebSocketFrameError("RSV bits must be zero");
        // Get opcode
        var opcodeNum = bitBuffer.getBits(BITS_OFFSET_OPCODE, BITS_LEN_OPCODE, false);
        WebSocketFrame.validateOpcode(opcodeNum);
        var opcode = opcodeNum;
        bufferRemainingBits -= BITS_LEN_OPCODE;
        // Get if the frame is masked
        var mask = intToBool(bitBuffer.getBits(BITS_OFFSET_MASK, BITS_LEN_MASK, false));
        bufferRemainingBits -= BITS_LEN_MASK;
        // Get the payload length
        var payloadLenLvl0Val = bitBuffer.getBits(BITS_OFFSET_PAYLOAD, PayloadLenLevelBits.LEVEL0, false);
        ...
        if (payloadLenLevel === PayloadLenLevel.LEVEL0)
            ...
            if (bufferRemainingBits < payloadAdditionalBitsLength)
                throw new WebSocketFrameError("Buffer length too small");
            payloadBytesLen = bitBuffer.getBits(BITS_OFFSET_PAYLOAD + PayloadLenLevelBits.LEVEL0, payloadAdditionalBitsLength);
        ...
        if (mask) {
            ...
            maskingKey = bitBuffer.getBits(BITS_OFFSET_PAYLOAD +
                PayloadLenLevelBits.LEVEL0 +
        ...
        if (opcode === WebSocketFrameOpcode.conn_close && hasCloseReasonCode) {
            ...
            var closeReasonNum = bitBuffer.buffer.readUInt16BE((PayloadLenLevelBits.LEVEL0 +
        ...
        if (payloadBytesLen > 0) {
            ...
            bitBuffer.buffer.copy(payloadData, 0, payloadStartByte, payloadStartByte + payloadBytesLen);
    ...
    WebSocketFrame.prototype.toBuffer = function () {
        ...
        bitBuffer.bigEndian = true;
        // Set fin flag
        bitBuffer.setBits(BITS_OFFSET_FIN, boolToInt(this.fin), BITS_LEN_FIN);
        // Set RSV flags
        bitBuffer.setBits(BITS_OFFSET_RSV, 0, BITS_LEN_RSV);
        // Set opcode field
        bitBuffer.setBits(BITS_OFFSET_OPCODE, this.opcode, BITS_LEN_OPCODE);
        // Set mask flag
        bitBuffer.setBits(BITS_OFFSET_MASK, boolToInt(this.mask), BITS_LEN_MASK);
        // Set payload length bits
        if (this.payloadLenLevel === PayloadLenLevel.LEVEL0) {
            bitBuffer.setBits(BITS_OFFSET_PAYLOAD, this.payloadBytesLen, PayloadLenLevelBits.LEVEL0);
        }
        else if (this.payloadLenLevel === PayloadLenLevel.LEVEL1) {
            bitBuffer.setBits(BITS_OFFSET_PAYLOAD, 126, PayloadLenLevelBits.LEVEL0);
            bitBuffer.setBits(BITS_OFFSET_PAYLOAD + PayloadLenLevelBits.LEVEL0, this.payloadBytesLen, PayloadLenLevelBits.LEVEL1);
        }
        else if (this.payloadLenLevel === PayloadLenLevel.LEVEL2) {
            bitBuffer.setBits(BITS_OFFSET_PAYLOAD, 127, PayloadLenLevelBits.LEVEL0);
            bitBuffer.setBits(BITS_OFFSET_PAYLOAD + PayloadLenLevelBits.LEVEL0, this.payloadBytesLen, PayloadLenLevelBits.LEVEL2);
        }
        else
            notReached();
        ...
        if (this.payloadData) {
            ...
            this.payloadData.copy(bitBuffer.buffer, bufferPayloadStart, 0, this.payloadBytesLen);
            ...
            if (this.mask) {
                assert(this.maskingKey);
                WebSocketFrame.applyMaskToBuffer(bitBuffer.buffer, this.maskingKey, bufferPayloadStartWithCloseCode, this.payloadBytesLen);
            }
        }
...
var WebSocketClient = /** @class */ (function (_super) {
    ...
    WebSocketClient.prototype.parseAndValidateUrl = function (urlStr) {
        ...
        if (!VALID_PROTOCOLS.includes(url.protocol))
            throw new WebSocketInitializationError("Invalid protocol '".concat(url.protocol, "'"));
        if (url.port.length == 0)
            url.port = DEFAULT_PORTS[url.protocol].toString();
        return url;
    };
    /**
    ...
    WebSocketClient.prototype.validateResHandshake = function (res, nonceB64) {
        ...
        var secWsAcceptClientHash = crypto.createHash("sha1");
    ...
    WebSocketClient.prototype.handleMessageFrame = function (frame) {
        assert(frame.opcode === WebSocketFrameOpcode.binary ||
            frame.opcode === WebSocketFrameOpcode.text);
